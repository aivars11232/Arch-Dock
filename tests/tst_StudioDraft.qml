...
TestCase {
    ...
    function test_mergeAcceptsEmptyInputs() {
        const fromNull = StudioDraft.merge(null, {
            opacity: 0.7
        });
        const withNull = StudioDraft.merge({
            opacity: 0.7
        }, null);
    ...
    function test_nestedMapsTrackIndependentPanelDrafts() {
        let drafts = {};
        drafts = StudioDraft.setNestedMap(drafts, "free-1", StudioDraft.setValue({}, "opacity", 0.5));
        drafts = StudioDraft.setNestedMap(drafts, "bottom", StudioDraft.setValue({}, "iconSize", 64));
    ...
    function test_captureRevisionPreservesTheFirstEditorSnapshot() {
        let revisions = StudioDraft.captureRevision({}, "bottom", 4);
        revisions = StudioDraft.captureRevision(revisions, "bottom", 9);
        revisions = StudioDraft.captureRevision(revisions, "side", 2);

        compare(StudioDraft.revisionFor(revisions, "bottom", 0), 4);
        compare(StudioDraft.revisionFor(revisions, "side", 0), 2);
        compare(StudioDraft.revisionFor(revisions, "missing", 7), 7);
    }

    function test_panelTransactionValuesSubmitOnlyTheScreenIndex() {
        const panel = {
            opacity: 0.6
        };
        const screen = {
        ...
        const candidate = StudioDraft.panelTransactionValues(panel, screen);

        compare(candidate.opacity, 0.6);
        compare(candidate.screen, 2);
        verify(!StudioDraft.hasValue(candidate, "screenId"));
        verify(!StudioDraft.hasValue(panel, "screen"));
        verify(!StudioDraft.hasValue(screen, "opacity"));
    }

    function test_transactionResultClassificationIsExplicit() {
        verify(StudioDraft.transactionSucceeded({
            success: true,
            status: "succeeded"
        }));
        verify(!StudioDraft.transactionSucceeded({
        ...
        verify(StudioDraft.transactionConflict({
            success: false,
        ...
        verify(!StudioDraft.transactionConflict(null));
    }
}
